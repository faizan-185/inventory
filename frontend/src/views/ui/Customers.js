...
import { getAllCustomers, createCustomer, updateCustomer, deleteCustomers } from '../../api/customer'
...
const Customers = () => {
    ...
    const addCustomer = async (event) => {
        ...
        if (btnText === 'Add') {
            setLoading(true);
            await createCustomer(name, category, reference, phone, address).then((result) => {
            ...
            setLoading(true);
            await updateCustomer(id, name, category, reference, phone, address).then((result) => {
                if (result.status === 200) {
                    setLoading(false);
